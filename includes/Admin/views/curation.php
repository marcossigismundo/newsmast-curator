<div class="nc-page-header">
    <div class="nc-page-title">
        ...
        <div class="nc-page-actions">
            ...
            <button class="nc-button nc-button-outline" id="nc-btn-cleanup" onclick="NC.openCleanupModal()">
                <span class="dashicons dashicons-trash"></span>
                <?php _e('Limpeza', 'newsmast-curator'); ?>
            </button>
        </div>
    </div>
...
<div class="nc-tabs">
    ...
    <button class="nc-tab" data-tab="explorar" onclick="NC.switchTab('explore')">
        <span class="dashicons dashicons-search" style="font-size:16px;width:16px;height:16px;vertical-align:middle;"></span>
        <?php _e('Explorar Tainacan', 'newsmast-curator'); ?>
    </button>
</div>

<!-- Tainacan Explorer Panel (hidden by default) -->
<div id="nc-explore-panel" style="display:none;">
    <div class="nc-card" style="margin-bottom:16px;">
        <div class="nc-card-body" style="padding:16px;">
            <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">
                <div class="nc-form-group" style="flex:0 0 auto;margin:0;">
                    <label class="nc-form-label"><?php _e('Fonte Tainacan', 'newsmast-curator'); ?></label>
                    <select id="nc-explore-source" class="nc-form-control" onchange="NC.exploreSourceChanged()">
                        <option value=""><?php _e('Selecione uma fonte...', 'newsmast-curator'); ?></option>
                    </select>
                </div>
                <div class="nc-form-group" style="flex:1;min-width:200px;margin:0;">
                    <label class="nc-form-label"><?php _e('Buscar no acervo', 'newsmast-curator'); ?></label>
                    <input type="text" id="nc-explore-search" class="nc-form-control" placeholder="<?php esc_attr_e('Palavras-chave...', 'newsmast-curator'); ?>">
                </div>
                <div class="nc-form-group" style="flex:0 0 auto;margin:0;">
                    <label class="nc-form-label"><?php _e('Ordenar', 'newsmast-curator'); ?></label>
                    <select id="nc-explore-orderby" class="nc-form-control">
                        <option value="date"><?php _e('Mais recentes', 'newsmast-curator'); ?></option>
                        <option value="title"><?php _e('Título', 'newsmast-curator'); ?></option>
                    </select>
                </div>
                <button class="nc-button nc-button-primary" onclick="NC.exploreBrowse(1)" style="margin-bottom:0;">
                    <span class="dashicons dashicons-search"></span> <?php _e('Buscar', 'newsmast-curator'); ?>
                </button>
            </div>
            <div id="nc-explore-facets" style="display:none;margin-top:12px;"></div>
        </div>
    </div>
    <div id="nc-explore-info" style="display:none;margin-bottom:12px;">
        <div class="nc-notice nc-notice-info" style="display:flex;justify-content:space-between;align-items:center;">
            <span><span class="dashicons dashicons-database"></span> <span id="nc-explore-total">0</span> <?php _e('itens no acervo', 'newsmast-curator'); ?></span>
            <span id="nc-explore-selected-info" style="display:none;">
                <strong id="nc-explore-selected-count">0</strong> <?php _e('selecionados para importar', 'newsmast-curator'); ?>
                <button class="nc-button nc-button-success nc-btn-sm" onclick="NC.exploreImportSelected()" style="margin-left:10px;">
                    <span class="dashicons dashicons-download"></span> <?php _e('Importar selecionados', 'newsmast-curator'); ?>
                </button>
            </span>
        </div>
    </div>
    <div id="nc-explore-results" class="nc-card">
        <div class="nc-card-body" style="padding:20px;text-align:center;color:var(--nc-text-light);">
            <span class="dashicons dashicons-search" style="font-size:48px;width:48px;height:48px;color:var(--nc-border);"></span>
            <p><?php _e('Selecione uma fonte Tainacan e clique em Buscar para explorar o acervo.', 'newsmast-curator'); ?></p>
            <p style="font-size:13px;"><?php _e('Você poderá selecionar os itens desejados e importá-los para curadoria.', 'newsmast-curator'); ?></p>
        </div>
    </div>
    <div id="nc-explore-pagination" style="display:none;margin-top:12px;text-align:center;"></div>
</div>
...
NC.switchTab = function(curated) {
    ...
    if (curated === 'explore') {
        jQuery('.nc-tab[data-tab="explorar"]').addClass('active');
        jQuery('#nc-explore-panel').show();
        jQuery('.nc-search-filters, #nc-advanced-filters, .nc-card:not(#nc-explore-results), #nc-floating-bar').hide();
        jQuery('#nc-btn-bulk-curate, #nc-btn-bulk-schedule, #nc-btn-add-to-collection, #nc-btn-triage').hide();
        NC.initExplorer();
        return;
    }

    // Restore normal curation view
    jQuery('#nc-explore-panel').hide();
    jQuery('.nc-search-filters, .nc-card').show();
...
NC._explorePage = 1;
NC._exploreInitialized = false;

NC.initExplorer = function() {
    if (NC._exploreInitialized) return;
    NC._exploreInitialized = true;

    // Populate Tainacan sources dropdown
    var $sel = jQuery('#nc-explore-source');
    Object.keys(NC._sourcesMap).forEach(function(id) {
        var s = NC._sourcesMap[id];
        if (s.connector_type === 'tainacan') {
            var label = NC.escapeHtml(s.name);
            if (s.config && s.config.search_terms) {
                label += ' (' + NC.escapeHtml(s.config.search_terms) + ')';
            }
...
NC.exploreSourceChanged = function() {
    var sourceId = jQuery('#nc-explore-source').val();
    if (!sourceId) {
        jQuery('#nc-explore-facets').hide().empty();
        return;
    }
    // Load Tainacan facets for explorer
    wp.apiFetch({path: ncData.apiUrl + '/sources/' + sourceId + '/tainacan-facets'}).then(function(data) {
        var facets = data.facets || [];
        if (facets.length === 0) { jQuery('#nc-explore-facets').hide().empty(); return; }
        var html = '<div class="nc-filters-grid">';
        facets.forEach(function(f) {
            if (!f.values || f.values.length === 0) return;
            html += '<div class="nc-filter-group">';
            html += '<label class="nc-filter-label">' + NC.escapeHtml(f.name) + '</label>';
            html += '<select class="nc-form-control nc-filter-input nc-explore-facet" data-slug="' + NC.escapeHtml(f.slug) + '">';
            html += '<option value="">Todos</option>';
            f.values.forEach(function(v) {
                if (v) html += '<option value="' + NC.escapeHtml(v) + '">' + NC.escapeHtml(v.length > 40 ? v.substring(0, 40) + '...' : v) + '</option>';
            });
            html += '</select></div>';
        });
        html += '</div>';
        jQuery('#nc-explore-facets').html(html).show();
    });
};
...
NC.exploreBrowse = function(page) {
    var sourceId = jQuery('#nc-explore-source').val();
    if (!sourceId) {
        NC.showNotice('warning', 'Selecione uma fonte Tainacan');
        return;
    }
    ...
    wp.apiFetch({
        ...
        jQuery('#nc-explore-info').show();
        jQuery('#nc-explore-total').text(res.total || 0);

        if (!res.items || res.items.length === 0) {
            jQuery('#nc-explore-results').html(
                '<div class="nc-card-body" style="padding:40px;text-align:center;color:var(--nc-text-light);">' +
        ...
        var html = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;padding:16px;">';
        res.items.forEach(function(item) {
            ...
            html += '<h4 class="nc-explore-tile-title">' + NC.escapeHtml(item.title.length > 60 ? item.title.substring(0, 60) + '...' : item.title) + '</h4>';
            if (item.author) html += '<div class="nc-explore-tile-meta">' + NC.escapeHtml(item.author) + '</div>';

            if (imported) {
                html += '<span class="nc-badge nc-badge-info" style="font-size:10px;">Já importado</span>';
            } else if (isSelected) {
                html += '<span class="nc-badge nc-badge-success" style="font-size:10px;">Selecionado</span>';
            }
        ...
        if (res.total_pages > 1) {
            var phtml = '<div style="display:flex;gap:6px;justify-content:center;align-items:center;flex-wrap:wrap;">';
...
NC.exploreImportSelected = function() {
    ...
    if (ids.length === 0) {
        NC.showNotice('warning', 'Selecione itens para importar');
        return;
    }

    var sourceId = jQuery('#nc-explore-source').val();
    NC.showNotice('info', 'Importando ' + ids.length + ' itens...');

    wp.apiFetch({
        path: ncData.apiUrl + '/items/import-selected',
        method: 'POST',
        data: { source_id: parseInt(sourceId), tainacan_ids: ids }
    }).then(function(res) {
        NC.showNotice('success', res.imported + ' itens importados! (' + res.skipped + ' já existiam)');
        NC._exploreSelectedIds = {};
        NC.exploreUpdateSelection();
        // Refresh current view to show imported badges
...
// ========== Cleanup Modal ==========
NC.openCleanupModal = function() {
    jQuery('#nc-cleanup-modal').remove();

    wp.apiFetch({path: ncData.apiUrl + '/items/cleanup-stats'}).then(function(stats) {
        var html =
        '<div id="nc-cleanup-modal" class="nc-modal-overlay">' +
            '<div class="nc-modal" style="max-width:560px;">' +
                '<div class="nc-modal-header">' +
                    '<h3 class="nc-modal-title"><span class="dashicons dashicons-trash" style="color:var(--nc-danger);"></span> Limpeza da Curadoria</h3>' +
                    '<button class="nc-modal-close" onclick="NC.closeModal(\'nc-cleanup-modal\')">&times;</button>' +
                '</div>' +
                '<div class="nc-modal-body">' +
                    '<div style="background:var(--nc-bg);border-radius:10px;padding:16px;margin-bottom:16px;">' +
                        '<h4 style="margin:0 0 12px;">Relatório da base</h4>' +
                        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px;">' +
                            '<div><strong>' + stats.total + '</strong> itens no total</div>' +
                            '<div><strong>' + stats.published_items + '</strong> já publicados</div>' +
                            '<div><strong>' + stats.uncurated + '</strong> não curados</div>' +
                            '<div><strong>' + stats.curated_no_pub + '</strong> curados sem publicação</div>' +
                            '<div><strong>' + stats.old_uncurated + '</strong> não curados há +30 dias</div>' +
                            '<div><strong>' + stats.with_pending + '</strong> com publicação pendente</div>' +
                        '</div>' +
                    '</div>' +

                    '<div style="display:flex;flex-direction:column;gap:10px;">' +
                        // Option 1: Delete published
                        '<div class="nc-cleanup-option" style="border:1px solid var(--nc-border);border-radius:8px;padding:14px;cursor:pointer;" onclick="NC.executeCleanup(\'published\')">' +
                            '<div style="display:flex;align-items:center;gap:10px;">' +
                                '<span class="dashicons dashicons-yes-alt" style="color:var(--nc-success);font-size:20px;width:20px;height:20px;"></span>' +
                                '<div><strong>Limpar itens já publicados</strong>' +
                                    '<div style="font-size:12px;color:var(--nc-text-light);">Remove ' + stats.published_items + ' itens que já foram publicados no Mastodon (sem publicações pendentes). Os posts no Mastodon não são afetados.</div>' +
                                '</div>' +
                            '</div>' +
                        '</div>' +

                        // Option 2: Delete old uncurated
                        '<div class="nc-cleanup-option" style="border:1px solid var(--nc-border);border-radius:8px;padding:14px;cursor:pointer;" onclick="NC.executeCleanup(\'old_uncurated\')">' +
                            '<div style="display:flex;align-items:center;gap:10px;">' +
                                '<span class="dashicons dashicons-clock" style="color:var(--nc-warning);font-size:20px;width:20px;height:20px;"></span>' +
                                '<div><strong>Limpar não curados antigos (+30 dias)</strong>' +
                                    '<div style="font-size:12px;color:var(--nc-text-light);">Remove ' + stats.old_uncurated + ' itens coletados há mais de 30 dias que nunca foram aprovados.</div>' +
                                '</div>' +
                            '</div>' +
                        '</div>' +

                        // Option 3: Delete ALL uncurated
                        '<div class="nc-cleanup-option" style="border:1px solid var(--nc-border);border-radius:8px;padding:14px;cursor:pointer;" onclick="NC.executeCleanup(\'all_uncurated\')">' +
                            '<div style="display:flex;align-items:center;gap:10px;">' +
                                '<span class="dashicons dashicons-dismiss" style="color:var(--nc-danger);font-size:20px;width:20px;height:20px;"></span>' +
                                '<div><strong>Limpar TODOS os não curados</strong>' +
                                    '<div style="font-size:12px;color:var(--nc-text-light);">Remove ' + stats.uncurated + ' itens da aba "Novos" que não foram aprovados. Ação irreversível.</div>' +
                                '</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
                '<div class="nc-modal-footer">' +
                    '<button class="nc-button nc-button-secondary" onclick="NC.closeModal(\'nc-cleanup-modal\')">Fechar</button>' +
                '</div>' +
            '</div>' +
        '</div>';
        jQuery('body').append(html);
        NC.openModal('nc-cleanup-modal');
    });
};

NC.executeCleanup = function(action) {
    var labels = {
        'published': 'Remover itens já publicados?',
        'old_uncurated': 'Remover itens não curados com mais de 30 dias?',
        'all_uncurated': 'ATENÇÃO: Remover TODOS os itens não curados? Esta ação é irreversível.'
    };

    if (!confirm(labels[action] || 'Confirma a limpeza?')) return;

    NC.showNotice('info', 'Executando limpeza...');
    NC.closeModal('nc-cleanup-modal');

    wp.apiFetch({
        path: ncData.apiUrl + '/items/cleanup',
        method: 'POST',
        data: { action: action }
    }).then(function(res) {
        NC.showNotice('success', res.deleted + ' itens removidos com sucesso!');
        // Refresh stats and view
        wp.apiFetch({path: ncData.apiUrl + '/items/stats'}).then(function(stats) {
            jQuery('#nc-uncurated-count').text(stats.uncurated || 0);
            jQuery('#nc-curated-count').text(stats.curated || 0);
        });
        ...
        NC.showNotice('error', 'Erro na limpeza: ' + (e.message || ''));
    });
};
</script>
